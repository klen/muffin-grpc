...
class Plugin(BasePlugin):
    ...
    def add_proto(self, path: t.Union[str, Path], build_dir: t.Union[str, Path] = None,
                  build_module: str = None):
        """Register/build the given proto file."""
        path = Path(path).absolute()
        self.proto_files.append(path)
        self.build_proto(
            path, build_dir=build_dir or self.cfg.build_dir, build_module=build_module)
    ...
    def build_proto(self, path: t.Union[str, Path], build_module: str = None,
        ...
        if build_module:
            _generate_file(build_dir / f"{ build_module }.py", *[
